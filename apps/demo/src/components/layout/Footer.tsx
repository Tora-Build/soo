...
export const Footer = () => {
  const { currentChainStatus, isLoading, error, notConfigured } =
    useIndexerStatus();
  const { rpcBlock, rpcConnected } = useChainStatus();
  const [showStatusHelp, setShowStatusHelp] = useState(false);

  const syncLag =
    rpcBlock && currentChainStatus
                    ...
                    <div className="space-y-3 font-mono text-xs text-muted">
                      <div>
                        ...
                        <p className="text-faint ml-4">
                          Historical data for orderbook, trades, and market
                          history.
                          <br />
                          <span className="text-faint">
                            Without: No orderbook visible
                          </span>
                        </p>
                      </div>
