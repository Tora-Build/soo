...
export type AmmHistoricalPriceCardProps = {
...
export function AmmHistoricalPriceCard({
  chainId,
  market,
}: AmmHistoricalPriceCardProps) {
  ...
  const {
    ...
    isSyncing,
    rawTradeCount,
  } = useAmmPriceHistory(chainId, market, interval);

  // When a selected interval has fewer than 2 candle buckets, there is
  // nothing meaningful to draw at that timeframe. Auto-switch to the
  // smallest interval that can produce at least 2 buckets so the user
  // is never stuck on a label that cannot be honored by the data.
  useEffect(() => {
    const currentCount = bucketCountByInterval[interval] ?? 0;
    if (currentCount >= 2) return;
    const smallestUsable = INTERVALS.find(
      (item) => (bucketCountByInterval[item.value] ?? 0) >= 2,
    );
    if (smallestUsable && smallestUsable.value !== interval) {
      setInterval(smallestUsable.value);
    }
  }, [bucketCountByInterval, interval]);
  ...
  const nowMarker = useMemo(() => {
    ...
    return { x: latestPoint.x, y: latestPoint.y };
  ...
  const handleResetView = () => {
    if (totalCandles === 0) return;
    setVisibleCount(Math.min(DEFAULT_VISIBLE_CANDLES, totalCandles));
    setPanOffset(0);
  };
  ...
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    setPanOffset(
      clamp(dragState.current.startPan + deltaCandles, 0, maxPanOffset),
    );
  };
  ...
  return (
    <div className="bg-raised/50 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-raised p-0.5">
            {(["yes", "no"] as const).map((side) => (
              <button
                key={side}
                onClick={() => setViewOutcome(side)}
                className={cn(
                  "px-2 py-1 text-xs font-bold uppercase transition-colors",
                  viewOutcome === side
                    ? "bg-raised text-ink"
          ...
          <div className="flex items-center gap-1 bg-raised p-0.5">
            {INTERVALS.map((item) => {
              const bucketCount = bucketCountByInterval[item.value] ?? 0;
              const isUsable = bucketCount >= 2;
              return (
                <button
                  key={item.value}
                  ...
                  title={
                    isUsable
                      ? undefined
                      : `Not enough trades for ${item.label} buckets yet`
                  }
                  className={cn(
      ...
      <div className="p-4 space-y-3">
        {isLoading ? (
          ...
          <div className="h-36 bg-raised flex items-center justify-center text-xs text-muted">
            Indexer is still syncing AMM history for this chain
          </div>
        ) : isError ? (
          <div className="h-36 bg-raised flex items-center justify-center px-4 text-center text-xs text-muted">
            {errorMessage ?? "Unable to load AMM price history right now"}
          </div>
        ) : series.length < 2 ? (
          <>
            <div className="h-36 bg-raised flex items-center justify-center text-xs text-muted">
              {rawTradeCount > 0
                ? `Only one ${interval} candle is available so far. Switch to a shorter interval or wait for more trades.`
                : "No AMM trades yet for historical chart"}
            </div>
            {rawTradeCount === 0 && (
              <p className="text-[10px] text-faint font-mono text-center mt-2">
                Market created recently — price history will build as trades
                occur
              </p>
            )}
          </>
          ...
          <>
            ...
            {isRefreshError ? (
              <div className="px-1 text-[11px] text-muted">
                Showing cached history. Latest refresh failed.
              </div>
            ) : null}
          </>
        )}
        ...
        <div className="flex items-center gap-4 text-xs overflow-x-auto whitespace-nowrap pb-1">
          ...
          <div className="inline-flex items-baseline gap-1 shrink-0">
            ...
            <span
              className={cn(
                "font-mono",
                deltaDisplay >= 0 ? "text-ink" : "text-muted",
              )}
            >
              {deltaDisplay >= 0 ? "+" : "-"}
              {formatCents(Math.abs(deltaDisplay))} (
              {changePctDisplay >= 0 ? "+" : "-"}
              {Math.abs(changePctDisplay).toFixed(1)}%)
            </span>
          </div>
          <div className="inline-flex items-baseline gap-1 shrink-0">
          ...
          <div className="inline-flex items-baseline gap-1 shrink-0">
            ...
            <span className="font-mono text-ink">
              {totalVolumeWad.toLocaleString(undefined, {
