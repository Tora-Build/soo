...
> Supersedes [`docs/archive/sooth_book-fork-plan.md`](../archive/sooth_book-fork-plan.md).
> Companion (archived): [`docs/archive/ladder-vs-density.md`](../archive/ladder-vs-density.md).
...
- `sooth-alpha/packages/contracts-core/src/SoothBook.sol` — matching engine, 677 LOC
...
**`base_token_mint` field — load-bearing (not dead storage).** Copied from `market_usdc_vault.mint` at lazy-init and validated against the protocol's canonical base-token mint constant. Every subsequent ix asserts `market_book.base_token_mint == market_usdc_vault.mint` (defense-in-depth, ~1 CU). Without the field the spec would have to dereference the vault on every ix to discover the mint — the stored copy lets us treat the book as an audit-readable provenance source.

**`BASE_TOKEN_MINT` constant** lives in `sooth-protocol-types::ids` and is `cfg!`-switched: `cfg(feature = "mainnet")` resolves to `USDC_MINT_MAINNET`, default resolves to `USDC_MINT_DEVNET`. W1 adds both pubkeys plus the `mainnet` feature flag on `sooth-protocol-types`.
...
//   mint = market_usdc_vault.mint (validated against `sooth_protocol_types::ids::BASE_TOKEN_MINT` at init)
...
fn buy(ctx, side: u8, tick: u16, amount: u128, escrow: bool, match_limit_arg: u32) -> Result<()> {
    ...
    // Defense-in-depth: book.base_token_mint must match the vault's mint every ix.
    // The init-only check above protects creation; this protects subsequent calls
    // against a hypothetical vault swap. ~1 CU.
    require_keys_eq!(
        ctx.accounts.market_book.base_token_mint,
        ctx.accounts.market_usdc_vault.mint,
...
**Fee transfer destination only** (buy path). `trade_positions.rs:467-477` (real fee CPI body; the `:50-52` and `:66-73` references in earlier drafts pointed at the file-level docstring, not the code) — destination changes from global `fee_pool_vault` (legacy) to per-market `market_fee_pool`. Signer = user (source is user's USDC ATA — user signs both `cost_usdc → market_vault` and `fee_usdc → market_fee_pool` transfers). `TradePositions` accounts struct replaces the `fee_pool_vault` field with `market_fee_pool`.

**Sell-path fee wiring (separate W2 sub-task).** This is **NOT** a destination redirect; `sell_positions.rs` currently has no fee code at all (no `fee_pool_vault` field, no `fee_wad` computation, no fee CPI). Scope: port EVM `FeeRouter._quoteFee` to the sell path (`netAmount = baseCost - fee` per `FeeRouter.sol:415-423`), add a second SPL transfer signed by `vault_authority` (must live in `sooth_market` since only it can sign for the market vault) routing `fee_usdc` to `market_fee_pool`, extend `SellPositions` accounts struct, update the AMM CU envelope. Tracked in W2 as its own sub-row in §11.
...
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **W1**  | Delete Monaco vendor + `vendor/`; new `sooth_book` skeleton; port `TickBitmap` library; `MarketBook` PDA + composite `order_id` codec; **add `SOOTH_BOOK_PROGRAM_ID` + `USDC_MINT_DEVNET` + `USDC_MINT_MAINNET` + cfg-switched `BASE_TOKEN_MINT` to `sooth-protocol-types::ids`** (with `mainnet` build feature); discriminator constants to `sooth-protocol-types::discriminators`                                                                                  | `cargo check` clean (both default + `--features mainnet`); bitmap unit tests pass (round-trip `find_next_down` / `find_next_up` against 1000 random patterns); `order_id` codec round-trip tests pass on all valid (side, tick, seq) triples; `BASE_TOKEN_MINT` resolves to the correct mint under each feature cfg                                                                                                                                                                                                       |
| **W2a** | Custody primitives + buy-side fee redirect: `OrderbookPosition` PDA; `require_sooth_book_cpi_parent` helper; all 5 filler-only ix on `sooth_market` (`fill_order`, `deposit_for_order`, `withdraw_for_order`, `credit_shares_for_order`, `debit_shares_for_order_before_deadline`); `init_market_fee_pool` ix on `sooth_launchpad`; `sooth_amm::trade_positions` fee destination redirect (lines 467-477); auth tests via test-only SoothBook-impersonating CPI stub | All filler ix reject direct call (negative test); reject wrong-discriminator parent ix (negative test); reject earlier-but-unrelated SoothBook ix in same tx (negative test against the scan-bypass attack); AMM buys fund per-market pool not global pool; deadline guards reject post-deadline mutations                                                                                                                                                                                                                |
| **W2b** | Sell-path fee port (separate sub-task — `sell_positions.rs` has zero fee code today): add `fee_wad` computation per EVM `FeeRouter._quoteFee` (`netAmount = baseCost - fee`); add second SPL transfer signed by `sooth_market::vault_authority` from `market_vault → market_fee_pool`; extend `SellPositions` accounts struct; update CU envelope                                                                                                                    | LiteSVM: every sell emits a non-zero fee transfer with the exact bps from `ProtocolConfig`; round-trip buy+sell on identical share counts collects the expected fee on both legs; CU per `sell_positions` ≤ 100k                                                                                                                                                                                                                                                                                                          |
...
| Fee dust rounding parity break vs EVM                                        | Med        | High   | Pinned in §7.3 (floor-on-sum). W4 acceptance includes golden-case fixtures against EVM `OrderEngine.sol:714-786`.                                                                                                                              |
...
- [`docs/archive/ladder-vs-density.md`](../archive/ladder-vs-density.md) (cap analysis; archived)
- [`docs/archive/sooth_book-fork-plan.md`](../archive/sooth_book-fork-plan.md) (superseded by this spec; archived)
