...
**4/5 production programs FULLY implemented (incl. redeem / dispute / LP-mint hook / dismiss-market / claim-refund / redeem-LP / graduation flip) + demo dapp end-to-end with 19/19 Playwright e2e specs green on a fresh Surfpool boot.**
...
| `sooth_book`                                     | Spec only. Gated on P1 (Monaco fork vs custom build) — research complete (`docs/research/monaco-investigation-week-01.md`), founder approval pending. The `/orderbook/:market` route renders a "SoothBook unavailable" card directly (no inner-hook crash); covered by spec 18.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **175 cargo tests** across the four programs (incl. cpi_redeem 4/4, cpi_dispute 3/3, LP-mint flow, dismiss-market, close_dismissed_position, claim_refund, redeem_lp, fee-accumulator + graduation flip).
- **49/49 SDK tests** across 15 files: smoke / sell-flow / claim-flow / refund / LP-redemption / graduation-progress / complete-set / redeem-request / operator-request / create-market / submit-failure / preflight / per-program error-classifier / LMSR.
...
- **19 Playwright on-chain e2e specs** against the LocalKeypairAdapter. 19/19 green on a fresh Surfpool boot. Beyond the original 12 (buy/sell/claim/mint/merge/redeem/attest-settle/trading-window/wallet-not-connected): 12 (create-market via UI), 13 (faucet UI), 14 (trade-to-graduate), 15 (dismiss-market), 16 (claim-refund), 17 (redeem-LP), 18 (orderbook gated state). Specs 08, 09, 14, 15, 16 use `surfnet_timeTravel` (with a `readOnchainClock`-relative delta so subsequent specs don't drift).
