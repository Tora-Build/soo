...
use crate::instructions::orderbook_common::{
    debit_shares, ensure_position_identity, require_before_deadline,
};
use crate::state::{Market, OrderbookPosition};

#[derive(Accounts)]
pub struct DebitSharesForOrderBeforeDeadline<'info> {
    #[account(
...
pub fn handler(
    ...
    debit_shares(&mut ctx.accounts.position, outcome, amount)
}
