...
pub fn update_locktime(market: &mut Market, lock_time: i64) -> Result<()> {
    let now = current_timestamp();
    update_market_locktime(market, lock_time, now)
}

pub fn update_locktime_to_now(market: &mut Market) -> Result<()> {
    let now = current_timestamp();
    update_market_locktime(market, now, now)
}

fn update_market_locktime(market: &mut Market, lock_time: i64, now: i64) -> Result<()> {
    require!(
        lock_time <= market.event_start_timestamp,
...
mod tests {
    ...
    fn test_market_update_success() {
        let mut market = Market {
            ...
            published: false,
            suspended: false,
            event_start_timestamp: 43041841910,
    ...
    #[test]
    fn test_market_update_fail_event_start_is_in_the_past() {
        let mut market = Market {
            authority: Default::default(),
            event_account: Default::default(),
            mint_account: Default::default(),
    ...
    fn test_market_update_failed_invalid_locktime() {
        let mut market = Market {
            ...
            published: false,
            suspended: false,
            event_start_timestamp: 1575975177,
            market_lock_order_behaviour: MarketOrderBehaviour::None,
            unclosed_accounts_count: 0,
            funding_account_bump: 0,
        };
