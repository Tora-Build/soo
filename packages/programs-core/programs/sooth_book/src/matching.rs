...
use sooth_market::state::{Market, OrderbookPosition};
use sooth_protocol_types::ids::BASE_TOKEN_MINT;

use crate::error::CoreError;
use crate::math::{MAX_TICK, NUM_TICKS};
use crate::state::{BookSide, InlineOrder, MarketBook, BOOK_SIDE_HEADER_SPACE, INLINE_ORDER_SPACE};
...
pub fn match_at_tick<'info>(
    ...
    require_keys_eq!(
        header.market,
        accounts.market.key(),
        CoreError::MissingCrossingBookSide
    );
    require!(
    ...
    while remaining > 0 && head < header.orders_len && match_limit > 0 {
        ...
        sooth_market::cpi::fill_order(
            CpiContext::new(
                ...
                sooth_market::cpi::accounts::FillOrder {
                    ...
                    taker_usdc_ata: accounts.taker_usdc_ata.to_account_info(),
                    maker_usdc_ata: remaining_accounts[bundle_base + 2].clone(),
                    taker_position: accounts.taker_orderbook_position.to_account_info(),
                    maker_position: remaining_accounts[bundle_base + 1].clone(),
                    taker: accounts.taker.to_account_info(),
                    protocol_config: accounts.protocol_config.to_account_info(),
...
pub fn decode_fill_return_data() -> Result<(u128, u128)> {
    let (program_id, data) = anchor_lang::solana_program::program::get_return_data()
        .ok_or(error!(CoreError::MissingFillReturnData))?;
    require_keys_eq!(
        program_id,
        sooth_market::ID,
        CoreError::WrongFillReturnProgram
    );
    let ret = sooth_market::FillReturnData::try_from_slice(&data)
        .map_err(|_| error!(CoreError::MalformedFillReturnData))?;
    Ok((ret.fee_base_delta, ret.taker_payout_delta))
}

const FILL_BUNDLE_LEN: usize = 5;
...
fn validate_fill_bundle<'info>(
    ...
    let book_side = remaining_accounts
        .get(base)
        .ok_or(error!(CoreError::MissingCrossingBookSide))?;
    let maker_position = remaining_accounts
        .get(base + 1)
        .ok_or(error!(CoreError::MakerAccountMismatch))?;
    ...
    if let Some(maker) = expected_maker {
        ...
        if !maker_position.data_is_empty() {
            ...
            let maker_position_account = OrderbookPosition::try_deserialize(&mut data_slice)
                .map_err(|_| error!(CoreError::MakerAccountMismatch))?;
            require_keys_eq!(
        ...
        let maker_usdc = TokenAccount::try_deserialize(&mut data_slice)
            .map_err(|_| error!(CoreError::MakerAccountMismatch))?;
        require_keys_eq!(maker_usdc.owner, maker, CoreError::MakerAccountMismatch);
        require_keys_eq!(
            maker_usdc.mint,
            BASE_TOKEN_MINT,
            CoreError::MakerAccountMismatch
...
fn read_book_side_header(info: &AccountInfo) -> Result<BookSideHeader> {
    ...
    let market = Pubkey::new_from_array(
        data[8..40]
            .try_into()
            .map_err(|_| error!(CoreError::MissingCrossingBookSide))?,
    );
    let side = data[40];
...
fn write_head_index(info: &AccountInfo, head: usize) -> Result<()> {
    let head = u32::try_from(head).map_err(|_| error!(CoreError::MathOverflow))?;
    let mut data = info.try_borrow_mut_data()?;
    data.get_mut(43..47)
        .ok_or(error!(CoreError::MissingCrossingBookSide))?
