...
//!
//! The current fee-distribution surface accepts an LP-yield vault but does
//! not define a signer for withdrawing from it. Redeem uses a distinct
//! singleton LP-yield authority PDA at seeds `[b"lp_yield_authority"]` and
//! requires the passed yield vault to be owned by it.
...
pub struct RedeemLp<'info> {
    /// Per-market AMM state. LP redemptions only open after graduation.
    #[account(
        constraint = amm_state.is_graduated @ SoothLaunchpadError::NotGraduated,
    )]
    pub amm_state: Box<Account<'info, AmmState>>,

    /// LP token mint. Supply before the burn is the denominator for the
    /// pro-rata yield calculation.
    #[account(mut)]
    pub lp_mint: Box<Account<'info, Mint>>,

    /// User's LP-token ATA. Burn source; the user signs as token authority.
    #[account(
        mut,
    ...
    /// USDC yield vault. Must be owned by `lp_yield_authority` so the PDA can
    /// sign the payout transfer.
    #[account(
        mut,
    ...
    /// Singleton PDA signer for LP-yield withdrawals.
    ...
    /// User's USDC token account. The mint must match the yield vault.
    #[account(
        mut,
        constraint = user_usdc_ata.mint == lp_yield_vault.mint @ SoothLaunchpadError::Unauthorized,
    )]
...
pub fn handler(ctx: Context<RedeemLp>, lp_amount: u64) -> Result<()> {
    require!(
        ...
        SoothLaunchpadError::NotGraduated
    );
    require!(lp_amount > 0, SoothLaunchpadError::ZeroLpAmount);

    let lp_supply = ctx.accounts.lp_mint.supply;
    require!(lp_supply > 0, SoothLaunchpadError::EmptyLpSupply);
